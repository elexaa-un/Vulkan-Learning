...
layout(location = 4) in vec4 fragPosLightSpace;  // Shadow Map: 光源裁剪空间坐标
...
// ===== Shadow Map 采样器 (set=0, binding=2) =====
// 使用 sampler2DShadow + 比较采样器，硬件自动执行 PCF
...
void main() {
    ...
    if (projCoords.z <= 1.0)
    {
        // 硬件 PCF: texture(samplerShadow, vec3(uv, depth))
        // GPU 自动对比 shadowMap 中的深度值和 projCoords.z
        // 使用 2x2 PCF（线性过滤 + 比较采样器自动实现）
        shadow = texture(shadowMap, projCoords);

        // 可选：调整阴影强度（让阴影不完全黑）
        shadow = mix(0.3, 1.0, shadow);  // 阴影区域保留 30% 环境光
    }

    // 应用阴影（环境光 + 直接光照部分都受阴影影响）
