...
namespace lve
{
    ...
    LveSnowSystem::LveSnowSystem(LveDevice &device, VkRenderPass renderPass,
                                 VkDescriptorSetLayout globalSetLayout,
                                 uint32_t maxParticles, LveModel *quadModel)
        : lveDevice(device), m_maxParticles(maxParticles), m_quadModel(quadModel)
    {
        std::cout << "[SnowSystem] Creating " << maxParticles << " snow particles..." << std::endl;
        ...
        LveBuffer staging(lveDevice, sizeof(SnowParticle), m_maxParticles,
        ...
        for (uint32_t i = 0; i < m_maxParticles; ++i) {
            ...
            p.velocity = glm::vec4(-1.0f + d01(rng) * 2.0f, -1.5f - d01(rng) * 3.0f,
                                   -1.0f + d01(rng) * 2.0f, d01(rng) * 10.0f);
            float size = 0.08f + d01(rng) * 0.12f;
            p.startColor = glm::vec4(1.0f, 1.0f, 1.0f, size);
            p.endColor = glm::vec4(1.0f, 1.0f, 1.0f, 0.1f);
            p.endColor.w = 3.0f + d01(rng) * 8.0f;
            p.color = p.startColor;
            staging.writeToIndex(&p, i);
        }
        staging.unmap();
        lveDevice.copyBuffer(staging.getBuffer(), m_particleBuffer->getBuffer(),
                             sizeof(SnowParticle) * m_maxParticles);

        std::cout << "[SnowSystem] Initialized " << m_maxParticles << " snow particles." << std::endl;
    }

    LveSnowSystem::~LveSnowSystem()
    {
    ...
    void LveSnowSystem::createBuffers()
    {
        m_particleBuffer = std::make_unique<LveBuffer>(
            lveDevice, sizeof(SnowParticle), m_maxParticles,
            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        m_uboBuffer = std::make_unique<LveBuffer>(
            lveDevice, sizeof(SnowUbo), 1,
            VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        m_uboBuffer->map();
    }

    void LveSnowSystem::loadTexture()
    {
        m_texture = std::make_unique<LveTexture>(lveDevice, "snow.png");
    }

    void LveSnowSystem::createDescriptorSets()
    {
    ...
    void LveSnowSystem::createComputePipeline()
    {
        ...
        if (vkCreatePipelineLayout(lveDevice.device(), &li, nullptr, &m_computePipelineLayout) != VK_SUCCESS)
            throw std::runtime_error("snow compute layout failed");

        auto code = readFile("shaders/particle_snow.comp.spv");
        ...
        if (vkCreateShaderModule(lveDevice.device(), &sm, nullptr, &mod) != VK_SUCCESS)
            throw std::runtime_error("snow comp shader module failed");
        ...
        if (vkCreateComputePipelines(lveDevice.device(), VK_NULL_HANDLE, 1, &pi, nullptr, &m_computePipeline) != VK_SUCCESS)
            throw std::runtime_error("snow compute pipeline failed");
        vkDestroyShaderModule(lveDevice.device(), mod, nullptr);
    }

    void LveSnowSystem::createGraphicsPipeline(VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout)
    {
        ...
        if (vkCreatePipelineLayout(lveDevice.device(), &li, nullptr, &m_graphicsPipelineLayout) != VK_SUCCESS)
            throw std::runtime_error("snow graphics layout failed");
        ...
        bindings[1].binding = 1; bindings[1].stride = sizeof(SnowParticle);
        ...
        m_graphicsPipeline = std::make_unique<LvePipeline>(
            lveDevice, "shaders/particle_snow.vert.spv", "shaders/particle_snow.frag.spv", cfg);
    }

    void LveSnowSystem::updateAndRender(FrameInfo &frameInfo,
                                         ...
                                         const WeatherPreset &weather, uint32_t activeCount)
    {
        ...
        ubo.gravity = -1.5f * weather.precipitationRate;
        ubo.windDirection = glm::vec3(weather.cloudWindVelocity.x, 0.0f, weather.cloudWindVelocity.y);
        ubo.windStrength = 0.8f * weather.precipitationRate;
        ...
        vkCmdDispatch(cmd, (activeCount + 127) / 128, 1, 1);
